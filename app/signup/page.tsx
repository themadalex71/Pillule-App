...
export default function SignUpPage() {
  ...
  const handleCreateAccount = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!firstName.trim() || !lastName.trim() || !username.trim() || !email.trim() || !password.trim()) {
      setErrorMessage("Remplis tous les champs pour creer ton compte.");
      return;
    }

    if (!firebaseReady) {
      setErrorMessage("Ajoute les variables Firebase dans .env.local avant de creer un compte.");
      return;
    }
    ...
    try {
      ...
      setUser(null);
      setInfoMessage("Compte cree. Verifie ton adresse mail puis connecte-toi. Redirection dans 5 secondes...");
    } catch (error: any) {
      setErrorMessage(error.message || "Creation du compte impossible.");
    } finally {
    ...
    return (
      <main className="flex min-h-[100dvh] items-center justify-center bg-[#fcf7f2] text-[#4c1d95]">
        <div className="text-center">
          ...
          <p className="text-sm text-[#6b21a8]">Preparation...</p>
        </div>
      </main>
    );
  }
          ...
          <form className="mt-4 space-y-3" onSubmit={handleCreateAccount}>
            <div className="grid grid-cols-2 gap-3">
              <label className="block">
                <span className="mb-1.5 block text-sm font-medium text-[#6f628f]">Nom</span>
              ...
              <label className="block">
                <span className="mb-1.5 block text-sm font-medium text-[#6f628f]">Prenom</span>
                <div className="flex items-center gap-3 rounded-2xl border border-[#ece4f7] bg-[#fcfbff] px-4 py-3">
                  <UserRound size={17} className="text-[#b19bd6]" />
                  <input
                    value={firstName}
            ...
            <label className="block">
              <span className="mb-1.5 block text-sm font-medium text-[#6f628f]">Adresse mail</span>
              <div className="flex items-center gap-3 rounded-2xl border border-[#ece4f7] bg-[#fcfbff] px-4 py-3">
            ...
            <label className="block">
              <span className="mb-1.5 block text-sm font-medium text-[#6f628f]">Mot de passe</span>
              <div className="flex items-center gap-3 rounded-2xl border border-[#ece4f7] bg-[#fcfbff] px-4 py-3">
            ...
            <label className="block">
              <span className="mb-1.5 block text-sm font-medium text-[#6f628f]">Nom d'utilisateur</span>
              <div className="flex items-center gap-3 rounded-2xl border border-[#ece4f7] bg-[#fcfbff] px-4 py-3">
            ...
            >
              {isSubmitting ? "Patiente..." : "Creer mon compte"}
            </button>
          </form>
        </section>
